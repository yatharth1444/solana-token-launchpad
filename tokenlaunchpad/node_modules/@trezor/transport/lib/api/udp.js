...
class UdpApi extends abstract_1.AbstractApi {
    constructor({ logger }) {
        ...
        this.enumerateAbortController = new AbortController();
    }
    listen() {
        if (this.listening)
            return;
    ...
    enumerate(signal) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const enumerateResult = yield Promise.all(paths.map(path => this.ping(path, signal).then(pinged => pinged ? { path, type: abstract_1.DEVICE_TYPE.TypeEmulator } : undefined))).then(res => res.filter(utils_1.isNotUndefined));
